...
def determine_frontier_path(state: SimulationState) -> None:
    ...
    if state.frontier_goal == state.goal["cell"]:
        ...
        if hasattr(state, "frontier_goal") and state.frontier_goal:
            fx, fy = cell_center(state.frontier_goal, state.world["cell_size_m"])
            dist_to_goal = math.hypot(fx - state.pose["x"], fy - state.pose["y"])
    
            # have not come to the goal（distance>0.15米）
